...
class Network:
    ...
    def inference(self, inputs, is_training):
        ...
        for i in range(self.config.num_layers):
            f_encoder_i = self. DLA_res_module(feature, inputs['xyz'][i], inputs['neigh_idx'][i], d_out[i],
        ...
        f_layer_fc2 = helper_tf_util.conv2d(f_decoder_list[-1], 32, [1, 1], 'fc2', [1, 1], 'VALID', True, is_training)
    ...
    def DLA_res_module(self, feature, xyz, neigh_idx, d_out, name, is_training):
        f_pc = helper_tf_util.conv2d(feature, d_out // 2, [1, 1], name + 'mlp1', [1, 1], 'VALID', True, is_training)
        f_pc = self.DLA_module(xyz, f_pc, neigh_idx, d_out, name + 'DLA', is_training)
    ...
    def DLA_module(self, xyz, feature, neigh_idx, d_out, name, is_training):
        d_in = feature.get_shape()[-1].value

        # position Encoding
        f_xyz = self.position_encoding(xyz, neigh_idx)
        f_xyz = tf.nn.leaky_relu(tf.layers.batch_normalization(f_xyz, -1, 0.99, 1e-6, training=is_training))
        f_xyz = helper_tf_util.conv2d(f_xyz, d_in, [1, 1], name + 'mlp1', [1, 1], 'VALID', True, is_training,
                                      activation_fn=tf.nn.relu)
        f_xyz = helper_tf_util.conv2d(f_xyz, d_in, [1, 1], name + 'mlp2', [1, 1], 'VALID', False, is_training,
                                      activation_fn=None)

        # self attention block
        f_agg = self.self_attention(feature, f_xyz, neigh_idx, d_out, name+'self_att', is_training)
        f_agg = tf.nn.leaky_relu(tf.layers.batch_normalization(f_agg, -1, 0.99, 1e-6, training=is_training))

        # attentive pooling
        f_neighbours = self.gather_neighbour(tf.squeeze(f_agg, axis=2), neigh_idx)
        f_concat = tf.concat([f_neighbours, f_xyz], axis=-1)
        f_out = self.attentive_pooling(f_concat, d_out, name + 'att_pooling_1', is_training)

        return f_out

    def position_encoding(self, xyz, neigh_idx):
    ...
    def self_attention(self, feature, xyz, neigh_idx, d_out, name, is_training):
        d_in = feature.get_shape()[-1].value
        feature_tile = tf.tile(feature, [1, 1, tf.shape(neigh_idx)[-1], 1])

        feature_q = helper_tf_util.conv2d(feature_tile, d_in, [1, 1], name + 'fc1', [1, 1], 'VALID', False, is_training,
                                          activation_fn=None)

        f_neighbours = self.gather_neighbour(tf.squeeze(feature, axis=2), neigh_idx)

        f_neighbours_k = helper_tf_util.conv2d(f_neighbours, d_in, [1, 1], name + 'fc2', [1, 1], 'VALID', False,
                                               is_training, activation_fn=None)

        f_neighbours_v = helper_tf_util.conv2d(f_neighbours, d_in, [1, 1], name + 'fc3', [1, 1], 'VALID', False,
                                               is_training, activation_fn=None)

        f_feature = f_neighbours_v + xyz

        f_sub = feature_q - f_neighbours_k + xyz

        f_sub = helper_tf_util.conv2d(f_sub, d_out, [1, 1], name + 'fc4', [1, 1], 'VALID', True,
                                      is_training, activation_fn=tf.nn.relu)

        f_sub = helper_tf_util.conv2d(f_sub, d_in, [1, 1], name + 'fc5', [1, 1], 'VALID', False,
                                      is_training, activation_fn=None)
        score = tf.nn.softmax(f_sub, axis=-1)
        attention = tf.multiply(score, f_feature)
        f_agg = tf.reduce_sum(attention, axis=2)
        batch_size = tf.shape(f_neighbours)[0]
        num_points = tf.shape(f_neighbours)[1]
        f_out = tf.reshape(f_agg, [batch_size, num_points, 1, d_in])
        return f_out
    ...
    @staticmethod
    def attentive_pooling(feature_set, d_out, name, is_training):
